...
const _patchedNavigate = navigateTo;
navigateTo = function(page, targetEmail) {
  if (page === 'usermgmt') {
    document.querySelectorAll('.nav-item').forEach(el => el.classList.toggle('active', el.dataset.page === page));
    document.getElementById('topbarTitle').textContent = 'User Management';
  ...
  if (page === 'myaccount') {
    document.querySelectorAll('.nav-item').forEach(el => el.classList.toggle('active', el.dataset.page === page));
    document.getElementById('topbarTitle').textContent = 'My Account';
    document.getElementById('pageContent').innerHTML = renderMyAccount();
    if (window.innerWidth < 768) closeSidebar();
    return;
  }
  _patchedNavigate(page, targetEmail);
};
...
// ── PATCH navigateTo to handle settings ──
...
// ── ADD DELETE BUTTON to project cards (patch) ──
// Project cards already call projectCard() — we patch it to add delete btn in admin mode
const __origProjectCard = projectCard;
function projectCard(p) {
  let html = __origProjectCard(p);
  if (isAdminMode()) {
    html = html.replace(
      '</button>\n      </div>\n    </div>\n  </div>',
...
// ── ADD DELETE to notes timeline (patch renderNotes) ──
const __origRenderNotes = renderNotes;
renderNotes = function(targetEmail) {
  let html = __origRenderNotes(targetEmail);
  if (isAdminMode()) {
    // inject delete buttons next to edit buttons in timeline
    html = html.replace(/onclick="openEditNoteModal\('(N[^']+)'\)">Edit<\/button>/g,
      (match, id) => `${match} <button class="delete-btn" onclick="deleteNote('${id}')">🗑</button>`);
  }
  return html;
};

// ── ADD DELETE to user management ──
const __origRenderUserMgmt = renderUserManagement;
renderUserManagement = function() {
  let html = __origRenderUserMgmt();
  if (isAdminMode()) {
    html = html.replace(/(<button class="btn btn-secondary btn-sm" onclick="openResetPasswordModal[^"]*"[^>]*>🔑 Reset PW<\/button>)/g,
      (match) => match + ` <button class="delete-btn" onclick="deleteUser(this.closest('.user-row-card').dataset.email)">🗑</button>`);
  }
  return html;
};
